...
impl Container {
    ...
    pub async fn stats_no_stream(&self, one_shot: Option<bool>) -> Result<models::ContainerStats200Response> {
        let mut ep = format!("/containers/{}/stats?stream=false", self.id);
        match one_shot {
            None => {}
            Some(param) => { ep.push_str(format!("&one-shot={}", param).as_str()); }
        }
        self.docker.get_json(&ep).await
    }}
